...
const MAINNET_URL =
  "https://eth-mainnet.g.alchemy.com/v2/nXfnz5sSmJgr640j4mCrUZaOtldbLzvb";
...
export const getPrice = async (inputAmount, poolAddress) => {
  ...
  const amountIn = ethers.parseEther(inputAmount.toString(), tokenDecimals0);

  const quotedAmountOut =
    await quoterContract.callStatic.quotedExactInputSingle(
      immutables.token0,
      immutables.token1,
      immutables.fee,
      amountIn,
  ...
  const amountOut = ethers.formatUnits(quotedAmountOut, tokenDecimals1);
